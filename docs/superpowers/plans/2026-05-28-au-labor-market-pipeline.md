...
- **As-built deviation:** Task 7/8's manual-fallback (`get_projections`/`get_state_projections` raising `FileNotFoundError` pointing at a path to drop a file) was SUPERSEDED during integration. Both now download automatically: projections from `bls.gov/emp/ind-occ-matrix/occupation.xlsx` (sheet Table 1.2), state from the Projections Central bulk presigned-CSV endpoint with hard-fail guardrails. See spec Section 14. All six sources auto-download; nothing is placed by hand.
